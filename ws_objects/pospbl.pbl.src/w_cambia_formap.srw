$PBExportHeader$w_cambia_formap.srw
$PBExportComments$Cambia la forma de pago del ticket.
forward
global type w_cambia_formap from window
end type
type dw_cajas from datawindow within w_cambia_formap
end type
type cb_4 from commandbutton within w_cambia_formap
end type
type dw_det_pago1 from datawindow within w_cambia_formap
end type
type cb_3 from commandbutton within w_cambia_formap
end type
type dw_det_pago from datawindow within w_cambia_formap
end type
type cb_2 from commandbutton within w_cambia_formap
end type
type cb_1 from commandbutton within w_cambia_formap
end type
type em_1 from editmask within w_cambia_formap
end type
type gb_1 from groupbox within w_cambia_formap
end type
end forward
...
global w_cambia_formap w_cambia_formap
...
event clicked;long ll_num,ll_cliente,ll_funcionario
string ls_estado,ls_estab,ls_ptoemi
double ldb_dif,valorc
datetime dt_fecha
dw_det_pago.accepttext()
ldb_dif = dw_det_pago.getitemnumber(dw_det_pago.rowcount(),"dif")
ll_num = long(em_1.text)
SELECT pv_nota_venta.estado,cod_vendedor,fecha_emision,cod_cliente,estab,ptoemi INTO :ls_estado,:ll_funcionario ,:dt_fecha,:ll_cliente ,:ls_estab,:ls_ptoemi
FROM pv_nota_venta  
	  ...
	  for i=1 to dw_det_pago.rowcount()
		...
		// ll_cliente=f_busca_cliente(ii_nrcaja,ll_num)
		 ...
		 if ll_cliente > 0 then
			valorc=dw_det_pago.object.aux[i]
			if f_ingresa_cxc(ii_nrcaja,ll_num,ll_funcionario,dt_fecha,valorc,ll_cliente,ls_estab,ls_ptoemi) < 0 then
				return 1 
				rollback;
			messagebox("","NO se puede grabar cuentas por cobrar")
			else
				commit;
						messagebox("","grabadas CXC")
			end if	
